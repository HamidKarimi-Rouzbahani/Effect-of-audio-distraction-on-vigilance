function TestVigDistraction_manual(Subj,percentage_target_cond,SaveAudio,chunk)
% Screen('Preference', 'SkipSyncTests',1);
% developed by Hamid Karimi-Rouzbahani on 23/March/2022
% modified by Hamid Karimi-Rouzbahani on 30/March/2022
...
% modified by Hamid Karimi-Rouzbahani on 1/Aug/2022 to work based on manual
% counterbalancing 
...
Mono_Stereo=2;  % mono=1; stereo =2
length_of_recording=(Trials_per_block+1)*trial_length/60; % length of recording in block

key_showing_target_colour = KbName('r'); % which key to press to see the cue

% Task parameters
% percentage_target_cond=[0.5 0.06]; % Frequency of targets across conditions
...
Blocks_per_condition=5; % Number of blocks per target frequency condition
...
Num_of_conditions=1; % Conditions are defined based on freq levels and audio recording
...
try
    ...
    for Block_Num=1:Num_of_conditions*Blocks_per_condition
        percentage_target=Condition_details(1,1);
        Audio_record=Condition_details(1,2);
        ...
        if Cued_color_in_block(1,Block_Num)==1
            target_color='RED';
        ...
        cue_time_indx=zeros(1,trial_length*(Trials_per_block+1));
        cue_count=0;
        cue_duration=[];
        for main_counter=1:trial_length*(Trials_per_block+1)
            ...
            % show the cue when necessary during the block
            flag=0;
            while keycode(key_showing_target_colour) == 1
                cue_time_indx(main_counter)=1;
                Screen('TextSize',wpoint, 30);
                if Cued_color_in_block(1,Block_Num)==1
                    target_color='RED';
                    message = ['\n\n\n\n\n ---  Respond to ',target_color, ' dots  ---\n\n\n\n\n'];
                ...
                Screen('Flip', wpoint ,0);
                [~, secs2,keycode,~] = KbCheck(-3);
                flag=1;
            end
            if flag==1
                cue_count=cue_count+1;
                cue_duration(cue_count)=secs2-secs;
            end
        ...
        save(['Subj_',num2str(Subj),'_Blk_',num2str(Block_Num),'_Chunk_',num2str(chunk),'_','_',Condition_string,...
            '_test_Distraction.mat'],'manually_deflected_top','top_events','top_events2','top_targets','top_targets2',...
            ...
            'Cued_color_in_block','cue_time_indx','cue_duration','Condition_details');
